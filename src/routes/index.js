...
const { pickupSchema, updateSchema, requestSchema, maintenanceTaskSchema, repairLogSchema, partsReceivedSchema } = require('../lib/validation');
...
router.get('/', async (req, res, next) => {
  try {
    ...
    chemicals.forEach((chemical) => {
      ...
      const combinedQty = inventory.shelfQty + inventory.lineQty;
      totalUnits += combinedQty;

      if (chemical.gallonsPerUnit) {
        totalGallons += combinedQty * chemical.gallonsPerUnit;
      }

      if (chemical.unit === 'BOX') {
        totalBoxes += combinedQty;
      } else if (chemical.unit === 'BARREL') {
        totalBarrels += combinedQty;
      } else if (chemical.unit === 'BUCKET') {
        totalBuckets += combinedQty;
      }
    });
    ...
    // Prepare chemicals with allowed locations and increments for frontend
    const chemicalsWithRules = sortedChemicals.map((chemical) => {
      const allowedLocations = getAllowedLocations(chemical);
      const shelfIncrement = getIncrementForLocation(chemical, 'SHELF');
      const lineIncrement = chemical.trackOnLine
        ? getIncrementForLocation(chemical, 'LINE')
        : null;
    ...
    res.render('index', {
      ...
      totals: {
        ...
        totalBarrels: totalBarrels.toFixed(2),
        totalBuckets: totalBuckets.toFixed(2),
      },
  ...
  try {
    ...
    for (const [chemicalId, pickupData] of Object.entries(pickups)) {
      ...
      try {
        ...
        const updatedInventory = await prisma.inventoryState.update({
          where: { chemicalId: chemical.id },
          data: {
            shelfQty: inventory.shelfQty + validated.qty,
          },
        });
...
router.post('/update', async (req, res, next) => {
  try {
    ...
    for (const [chemicalId, updateData] of Object.entries(updates)) {
      // Skip if qty is missing/empty
      if (updateData.qty === undefined || updateData.qty === '' || updateData.qty === null) {
        continue;
      }
  ...
  try {
    ...
    for (const pickup of validPickups) {
      ...
      await prisma.inventoryState.update({
        where: { chemicalId: chemicalId },
        data: {
          shelfQty: inventory.shelfQty + qty,
        },
      });
...
router.get('/maintenance', async (req, res, next) => {
  try {
    ...
    const parts = await prisma.partsReceived.findMany({
      orderBy: { date: 'desc' },
      take: 50,
    });
...
router.post('/maintenance/tasks', async (req, res, next) => {
  try {
    const data = maintenanceTaskSchema.parse(req.body);
...
// PATCH /maintenance/tasks/:id/complete - Mark task as completed
router.patch('/maintenance/tasks/:id/complete', async (req, res, next) => {
  try {
...
// DELETE /maintenance/tasks/:id - Delete task
router.delete('/maintenance/tasks/:id', async (req, res, next) => {
  try {
...
// POST /maintenance/repairs - Create repair log
router.post('/maintenance/repairs', async (req, res, next) => {
  try {
...
// POST /maintenance/parts - Create parts received entry
router.post('/maintenance/parts', async (req, res, next) => {
  try {
    const data = partsReceivedSchema.parse(req.body);
    const part = await prisma.partsReceived.create({
      data: {
        description: data.description,
